	...
	let onCreate=()=>{
		...
		$('#a_go_join').click(()=>{
			...
			/*$('#form_join').append('<h1 id="temp">추가된 노드<br/>'
					+'<a id="atag" href = "#">추가된 A 태그</a></h1>')*/
			/*$('<h1 id="temp">추가된 노드<br/>'
					+'<a id="atag" href = "#">추가된 A 태그2</a></h1>')		//appendTO =(append 거꾸로)
					.appendTo('#form_join')*/
            $('head')
            .html('<head>'+
                    '    <meta charset="utf-8">'+
            ...
            $('<button>',{
				...
				click : e=>{
					e.preventDefault();
					let data = {cid: $('#userid').val(), pw : $('#password').val()}
					alert('전송 아이디&비번 :' + data.cid)
					$.ajax({
						url : _+'/user/join',								//시작
						type : 'POST',										//패턴이기 때문에 외워서 타이핑이나 카피해도 됨
						dataType : 'json',								// 대신 값은 맞춰줘야함
						data : JSON.stringify(data),				//끝
						contentType : 'application/json',
						success : d => {
							alert('AJAX 성공아이디 : '+d.cid+', 성공비번 : '+d.pw)
						},
						error : e =>{
							alert('AJAX 조짐 ㅅㄱ')
						}
